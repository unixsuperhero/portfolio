...
`createPrPoller({ db, gh, ... })` returns `{ tick, start, stop, status, refresh, fetchOne }`. `db` is a `@portfolio/db` `Store` (or anything shaped like `store.github` + `store.settings`); `gh` is `{ graphql(query) }`, normally `ghGraphql` wrapped so tests can inject a recording fake instead.

- **`tick()`** — one cycle: the lists request (mine + review_requested + rateLimit), then, only if `rateLimit.remaining >= 200`, a second request for every watched PR not already in those lists (up to 25). Never more than 2 GraphQL requests. Each returned node is parsed, ignored-check patterns (global + per-PR) are applied, the row is upserted, and `diffPr` runs against the previous row to append events. `rateLimit.remaining < 200` schedules a skip until `resetAt`; a thrown error backs off exponentially up to 15 minutes.
...
- **`fetchOne({ owner, repo, number })`** — one GraphQL request for exactly one PR (used by `POST /api/prs/watch` when the URL isn't tracked yet).
