...
| `diff.ts` | `checksSummary(checks, ignoredPatterns)` — over non-ignored checks only, `failure`/`cancelled` > `pending` > `success`, `"none"` when nothing counts; `isIgnored(name, patterns)` — case-insensitive glob match (`*`); `applyIgnored(checks, patterns)`; `diffPr(previous, next, ignoredPatterns, at?)` — drafts `PrEvent`s for state, review, comment-count, and effective check-summary changes |
...
- **`tick()`** — one cycle: the lists request (mine + review_requested + rateLimit) once per `settings.github_dirs` entry, with `gh` run from that directory (`{ cwd }`), or once with no cwd when the list is empty. A PR belongs to the first directory that returns it (`source_dir`). Then, only if the lowest `rateLimit.remaining >= 200`, one request per directory for watched PRs not already in the lists (up to 25 each, grouped by `source_dir`). A directory that throws is skipped and named in `status().error`; the tick fails only when every directory throws. Each returned node is parsed, rules from `github_ignored_check_rules` whose exact `owner/repo` matches are applied, and the row is upserted without changing the user-owned `ignored` flag. `diffPr` appends events except for hidden PRs. Matching check failures remain stored and marked ignored but do not affect the effective summary. Hidden watched PRs are not sent in the watched batch. `rateLimit.remaining < 200` schedules a skip until `resetAt`; a thrown error backs off exponentially up to 15 minutes.
